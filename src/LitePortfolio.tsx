  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        ...
        <p className="text-[11px] font-black uppercase tracking-[0.32em] text-primary">{eyebrow}</p>
      </div>
      <h2 className="text-4xl font-black uppercase tracking-tight text-white sm:text-5xl">{title}</h2>
      {subtitle ? <p className="max-w-3xl text-base leading-relaxed text-slate-300">{subtitle}</p> : null}
    </div>
  );
}
...
function LiteCard({
  ...
  return (
    <div
      className={`rounded-[2rem] border border-white/10 bg-[linear-gradient(180deg,rgba(18,28,32,0.92)_0%,rgba(8,14,17,0.96)_100%)] p-6 shadow-[0_24px_80px_rgba(0,0,0,0.28)] backdrop-blur-xl ${className}`}
    >
      {children}
    </div>
  );
...
function ArtifactPlaceholder({
  ...
  return (
    <button
      type="button"
      onClick={onPreview}
      className="group flex h-full min-h-[15rem] w-full flex-col items-center justify-center gap-4 rounded-[1.5rem] border border-primary/20 bg-[radial-gradient(circle_at_top,rgba(6,249,249,0.12),transparent_55%),linear-gradient(180deg,rgba(5,12,14,0.92)_0%,rgba(5,9,11,1)_100%)] p-6 text-center transition-colors hover:border-primary/45"
    >
      ...
      <div className="space-y-2">
        ...
        <p className="text-sm leading-relaxed text-slate-400">Tap to load the video preview only when you need it.</p>
      </div>
    </button>
  );
}

function LiteArtifactCard({ project }: { project: Artifact }) {
  const [loadVideo, setLoadVideo] = useState(false);

  return (
    <LiteCard className="h-full">
      <div className="mb-5 flex flex-wrap gap-2">
        {project.tags.map((tag) => (
          <span
            key={tag}
            className="rounded-full border border-primary/20 bg-primary/10 px-3 py-1 text-[10px] font-black uppercase tracking-[0.18em] text-primary"
          >
            {tag}
          </span>
        ))}
      ...
      <div className="mb-6 overflow-hidden rounded-[1.5rem] border border-white/10 bg-black/30">
        {project.type === 'image' ? (
          <img
            src={project.src}
            alt={project.title}
          ...
          <ArtifactPlaceholder project={project} onPreview={() => setLoadVideo(true)} />
        )}
      </div>

      <div className="space-y-3">
        <h3 className="text-2xl font-black uppercase tracking-tight text-white">{project.title}</h3>
        <p className="text-sm leading-relaxed text-slate-400">{project.role}</p>
      </div>
    </LiteCard>
  );
}
...
export default function LitePortfolio({ experienceMode, isMobileDevice }: LitePortfolioProps) {
  ...
  return (
    <div className="bg-background-dark text-white selection:bg-primary/30 selection:text-primary">
      ...
      <div className="fixed inset-0 pointer-events-none opacity-100">
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_top,rgba(6,249,249,0.12),transparent_36%),radial-gradient(circle_at_bottom_right,rgba(168,85,247,0.12),transparent_28%),linear-gradient(180deg,#071014_0%,#05090b_100%)]" />
      ...
      <nav className="fixed top-0 z-50 w-full border-b border-white/5 bg-background-dark/75 backdrop-blur-xl">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] font-black uppercase tracking-[0.24em] text-slate-500">
                {experienceMode === 'simple' ? 'Lite Creative Mode' : 'Loading Immersive Mode'}
              </p>
            </div>
          </div>
          <a
            href="mailto:luwiyeyz@gmail.com"
            className="rounded-full border border-white/10 bg-white/5 px-5 py-2 text-[10px] font-black uppercase tracking-[0.2em] text-white transition-colors hover:border-primary hover:bg-primary hover:text-background-dark"
          >
            Email Me
          </a>
        </div>
      ...
      <main className="relative z-10">
        <section className="px-6 pt-32 pb-20 md:pt-40">
          <div className="mx-auto max-w-7xl">
            <div className="grid gap-8 lg:grid-cols-[1.3fr_0.7fr] lg:items-end">
              <LiteCard className="overflow-hidden">
                <div className="space-y-8">
                  <div className="inline-flex items-center gap-2 rounded-full border border-primary/20 bg-primary/10 px-4 py-2 text-[10px] font-black uppercase tracking-[0.24em] text-primary">
                    <Activity className="size-4" />
                    Mobile-Optimized Creative Portfolio
                  </div>
              ...
              <LiteCard className="h-full">
                <div className="space-y-6">
                  <div className="rounded-[1.5rem] border border-primary/15 bg-primary/8 p-5">
                    <p className="text-[11px] font-black uppercase tracking-[0.28em] text-primary">Performance Note</p>
                  ...
                  <div className="rounded-[1.5rem] border border-white/10 bg-black/20 p-5 text-sm leading-relaxed text-slate-300">
                    {isMobileDevice
        ...
        <section className="px-6 py-16">
          <div className="mx-auto max-w-7xl space-y-10">
            <LiteSectionTitle
              ...
              subtitle="The same technical focus areas from the immersive portfolio, presented in a lighter creative layout."
            />
            <div className="grid gap-6 md:grid-cols-3">
              {[Bot, BarChart3, Globe].map((Icon, index) => (
                ...
                >
                  <LiteCard className="h-full">
                    <Icon className={`mb-6 size-11 ${index === 1 ? 'text-violet-400' : 'text-primary'}`} />
        ...
        <section className="px-6 py-16">
          <div className="mx-auto max-w-5xl space-y-10">
            ...
            {certificates.map((certificate) => (
              <LiteCard key={certificate.title}>
        ...
        <section id="about" className="px-6 py-16">
          <div className="mx-auto grid max-w-7xl gap-8 lg:grid-cols-[0.9fr_1.1fr] lg:items-center">
            ...
            <LiteCard>
              <p className="text-[11px] font-black uppercase tracking-[0.28em] text-primary">Profile.Initialize()</p>
        ...
        <section id="contact" className="px-6 pt-16 pb-24">
          <div className="mx-auto max-w-7xl space-y-12">
            <LiteCard className="text-center">
              <p className="text-[11px] font-black uppercase tracking-[0.28em] text-primary">Contact.Sync()</p>
              <h2 className="mt-5 text-5xl font-black uppercase tracking-tight text-white sm:text-6xl">
                Let&apos;s <span className="text-primary">Sync</span>
              </h2>
            ...
            <div className="flex flex-col gap-5 border-t border-white/10 pt-8 text-[10px] font-black uppercase tracking-[0.28em] text-slate-500 md:flex-row md:items-center md:justify-between">
              <p>Copyright 2026 Zia Louise Mariano. Mobile-optimized creative mode active.</p>
