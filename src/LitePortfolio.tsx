...
function LiteSectionTitle({
  ...
  const isMobileMode = experienceMode === 'mobile';

  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        <div className={`h-px w-12 ${isMobileMode ? 'bg-white/20' : 'bg-primary'}`} />
        <p
          className={`text-[11px] font-black uppercase tracking-[0.32em] ${
            isMobileMode ? 'text-sky-200' : 'text-primary'
          }`}
        >
          {eyebrow}
        </p>
      ...
      {subtitle ? (
        <p className={`max-w-3xl text-base leading-relaxed ${isMobileMode ? 'text-slate-300' : 'text-slate-300'}`}>
          {subtitle}
        </p>
      ) : null}
    </div>
...
function LiteCard({
  ...
  return (
    <div
      className={`rounded-[2rem] border p-5 backdrop-blur-xl sm:p-6 ${
        isMobileMode
...
function ArtifactPlaceholder({
  ...
  return (
    <button
      type="button"
      onClick={onPreview}
      className={`group flex h-full min-h-[15rem] w-full flex-col items-center justify-center gap-4 rounded-[1.5rem] border p-5 text-center transition-colors sm:p-6 ${
        isMobileMode
    ...
    >
      ...
      <div className="space-y-2">
        ...
        <p className={`text-sm leading-relaxed ${isMobileMode ? 'text-slate-300' : 'text-slate-400'}`}>
    ...
    <LiteCard className="h-full" experienceMode={experienceMode}>
      ...
      <div
        className={`mb-6 overflow-hidden rounded-[1.5rem] border ${
          isMobileMode ? 'border-white/8 bg-slate-950/40' : 'border-white/10 bg-black/30'
        }`}
      >
        {project.type === 'image' ? (
          <img
      ...
      <div className="space-y-3">
        <h3 className="text-xl font-black uppercase tracking-tight text-white sm:text-2xl">{project.title}</h3>
        <p className={`text-sm leading-relaxed ${isMobileMode ? 'text-slate-300' : 'text-slate-400'}`}>
          {project.role}
        </p>
      </div>
...
function ArtifactGroup({
  ...
  return (
    <section className="space-y-6">
      <div className="space-y-2">
        <h3 className="text-xl font-black uppercase tracking-tight text-white sm:text-2xl">{title}</h3>
        <p className={`max-w-3xl text-sm leading-relaxed ${isMobileMode ? 'text-slate-300' : 'text-slate-400'}`}>
          {subtitle}
        </p>
      </div>
...
export default function LitePortfolio({ experienceMode, isMobileDevice }: LitePortfolioProps) {
  ...
  const pageClassName = isMobileMode
  ...
  const backdropGradientClassName = isMobileMode
  ...
  const backdropPatternClassName = isMobileMode
  ...
  const navClassName = isMobileMode
  ...
  const emailButtonClassName = isMobileMode
    ? 'w-full rounded-full border border-white/10 bg-white/5 px-5 py-2 text-center text-[10px] font-black uppercase tracking-[0.2em] text-white transition-colors hover:border-sky-300/40 hover:bg-sky-300/12 hover:text-sky-50 sm:w-auto'
  ...
  const notePanelClassName = isMobileMode
    ? 'rounded-[1.5rem] border border-sky-300/15 bg-sky-300/8 p-5'
    : 'rounded-[1.5rem] border border-primary/15 bg-primary/8 p-5';

  const statCardClassName = isMobileMode
    ? 'rounded-[1.5rem] border border-white/8 bg-white/[0.04] p-5'
    : 'rounded-[1.5rem] border border-white/10 bg-white/5 p-5';

  const proofIconClassName = isMobileMode ? 'text-sky-200' : 'text-primary';
  const modeLabel = isMobileMode ? 'Mobile Mode' : 'Loading Live 3D Mode';
  const performanceLabel = isMobileMode ? 'Mobile Note' : 'Loading Note';
  const performanceCopy = isMobileMode
    ? 'This version keeps the same projects and story, but uses a calmer backdrop, lighter visuals, and on-demand media loading for smoother browsing on phones and tablets.'
    : 'The live 3D portfolio is loading. Switch to Mobile anytime for the faster lightweight presentation.';
  ...
  return (
    <div className={pageClassName}>
      ...
      <nav className={navClassName}>
        <div className="mx-auto flex max-w-7xl flex-col items-start gap-3 px-4 py-4 sm:h-20 sm:flex-row sm:items-center sm:justify-between sm:px-6 sm:py-0">
          <div className="flex items-center gap-3">
      ...
      <main className="relative z-10">
        <section className="px-4 pt-30 pb-16 sm:px-6 sm:pt-32 sm:pb-20 md:pt-40">
          <div className="mx-auto max-w-7xl">
            <div className="grid gap-8 lg:grid-cols-[1.3fr_0.7fr] lg:items-end">
              <LiteCard className="overflow-hidden" experienceMode={experienceMode}>
                <div className="space-y-6 sm:space-y-8">
                  <div className="space-y-5">
                    <h1 className="max-w-5xl text-4xl font-black uppercase leading-[0.92] tracking-tight sm:text-6xl lg:text-8xl">
                      {isMobileMode ? (
                        <>
                  ...
                  <div className={notePanelClassName}>
                    <p
                      className={`text-[11px] font-black uppercase tracking-[0.28em] ${
                        isMobileMode ? 'text-sky-100' : 'text-primary'
                      }`}
                    >
                      {performanceLabel}
                    </p>
        ...
        <section className="px-4 py-14 sm:px-6 sm:py-16">
          <div className="mx-auto max-w-7xl space-y-10">
            <LiteSectionTitle
              eyebrow={isMobileMode ? 'Mobile Snapshot' : 'Proof Layer'}
              title="Core Strengths"
              subtitle={
                isMobileMode
                  ? 'The same technical focus areas from the immersive portfolio, reframed in a cleaner mobile-friendly presentation.'
                  ...
                  <LiteCard className="h-full" experienceMode={experienceMode}>
                    <Icon
                      className={`mb-6 size-11 ${
                        index === 1 ? (isMobileMode ? 'text-sky-200' : 'text-violet-400') : proofIconClassName
                      }`}
                    />
        ...
        <section id="work" className="px-4 py-14 sm:px-6 sm:py-16">
          <div className="mx-auto max-w-7xl space-y-14">
            <LiteSectionTitle
              eyebrow={isMobileMode ? 'Selected Work' : 'Artifact Index'}
        ...
        <section className="px-4 py-14 sm:px-6 sm:py-16">
          <div className="mx-auto max-w-7xl space-y-10">
            <LiteSectionTitle
              eyebrow={isMobileMode ? 'Portfolio Highlights' : 'Highlight Set'}
        ...
        <section className="px-4 py-14 sm:px-6 sm:py-16">
          <div className="mx-auto max-w-5xl space-y-10">
            <LiteSectionTitle
              eyebrow="Credentials"
              title={isMobileMode ? 'Professional Credentials' : 'Neural Credentials'}
        ...
        <section id="stack" className="px-4 py-14 sm:px-6 sm:py-16">
          <div className="mx-auto max-w-7xl space-y-10">
            <LiteSectionTitle
              eyebrow={isMobileMode ? 'Capability Map' : 'Stack Registry'}
        ...
        <section id="about" className="px-4 py-14 sm:px-6 sm:py-16">
          <div className="mx-auto grid max-w-7xl gap-8 lg:grid-cols-[0.9fr_1.1fr] lg:items-center">
            ...
            <LiteCard experienceMode={experienceMode}>
              <p className="text-[11px] font-black uppercase tracking-[0.28em] text-primary">
                {isMobileMode ? 'Profile' : 'Profile.Initialize()'}
              </p>
        ...
        <section className="px-4 py-14 sm:px-6 sm:py-16">
          <div className="mx-auto max-w-5xl space-y-10">
            <LiteSectionTitle
              eyebrow={isMobileMode ? 'Journey' : 'Journey Log'}
              title={isMobileMode ? 'Creative to Technical Journey' : 'Creative to Technical'}
        ...
        <section id="contact" className="px-4 pt-14 pb-24 sm:px-6 sm:pt-16">
          <div className="mx-auto max-w-7xl space-y-12">
            <LiteCard className="text-center" experienceMode={experienceMode}>
              <p className="text-[11px] font-black uppercase tracking-[0.28em] text-primary">
                {isMobileMode ? 'Contact' : 'Contact.Sync()'}
              </p>
              <h2 className="mt-5 text-4xl font-black uppercase tracking-tight text-white sm:text-6xl">
                Let&apos;s <span className="text-primary">{isMobileMode ? 'Connect' : 'Sync'}</span>
              </h2>
            ...
            <div className="flex flex-col gap-5 border-t border-white/10 pt-8 text-[10px] font-black uppercase tracking-[0.28em] text-slate-500 md:flex-row md:items-center md:justify-between">
              <p>
                ...
                {isMobileMode ? 'Mobile mode active.' : 'Live 3D portfolio preview loading.'}
              </p>
